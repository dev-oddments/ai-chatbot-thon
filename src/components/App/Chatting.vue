  ...
  <main>
    <div id="chatArea">
      ...
      <section class="container chat-container">
        ...
        <!-- Messages Table -->
        <section v-else>
          <table v-for="(m, mI) in messages" class="message" :key="mI">
            <tr class="mb-3">
              <!-- My message -->
              <div class="msg-my">
                <small class="timestamp">{{m.queryResult.timestamp}}</small>
                <Bubble :text="m.queryResult.queryText" from="me" />
              </div>
            </tr>
            <tr v-if="m.queryResult.fulfillmentMessages">
              <div class="chat-item mb-3">
                <img
                  class="chat-item-thumbnail"
                  src="https://tva1.sinaimg.cn/large/006y8mN6gy1g7ovs8etcaj3069069wen.jpg"
                />
                <!-- Dialogflow Components -->

                <div class="chat-item-contents">
                  <div class="chat-item-title">다산콜센터</div>
                  <tr
                    class="component"
                    v-for="(component, i) in m.queryResult.fulfillmentMessages"
                    :key="i"
                  >
                    <!-- Text (https://cloud.google.com/dialogflow/docs/reference/rest/v2beta1/projects.agent.intents#Text) -->
                    <Bubble
                      :text="component.text.text[0]"
                      v-if="component.text&& component.text.text[0]"
                    />
                    ...
                    <!-- custom card -->
                    <div class="list" v-if="component.basicCard">
                      <ListItem
                        class="pt-0"
                        :title="component.basicCard.formattedText"
                        :imageTitle="component.basicCard.image.accessibilityText"
                        :description="component.basicCard.subtitle"
                        :imageUri="component.basicCard.image.imageUri"
                        :uri="component.basicCard.buttons[0].openUriAction.uri"
                      />
                    </div>
                  ...
                  <section
                    v-if="m.queryResult.webhookPayload && m.queryResult.webhookPayload.google"
                  >
                    <tr
                      class="component"
                      v-for="(component, i) in m.queryResult.webhookPayload.google.richResponse.items"
                      :key="i"
                    >
                    ...
                    <tr
                      class="component"
                      v-for="(component, i) in m.queryResult.webhookPayload.google.systemIntent"
                      :key="i"
                    >
    ...
    <ChatInput @submit="send">
      ...
      -->
      <template v-if="suggestions.text_suggestions">
        <Suggestion
...
export default {
    ...
    suggestions() {
      if (this.messages.length > 0) {
        ...
        if (
          ...
          for (let component in last_message.queryResult.webhookPayload
            .google) {
            if (
  ...
  methods: {
    send(q) {
      ...
      fetch(this.config.app.gateway + "/" + this.session, {
        method: "POST",
        ...
        .then(response => {
          if (!response.error) {
            response.queryResult.timestamp = `${timestamp.getHours()}:${timestamp.getMinutes()}`;
    ...
    handle(response) {
      ...
      for (let component in response.queryResult.fulfillmentMessages) {
        ...
        if (response.queryResult.fulfillmentMessages[component].simpleResponses)
          text =
      ...
      if (
        ...
        for (let component in response.queryResult.webhookPayload.google) {
          if (
            response.queryResult.webhookPayload.google[component].simpleResponse
          )
            text =
