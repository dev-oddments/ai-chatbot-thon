<template>
  <div class="overlay">
    ...
    <!-- <img
      :alt="app.displayName"
      class="app-icon"
      src="https://console.dialogflow.com/api-client/assets/img/logo-short.png"
      v-else
    />-->
    ...
    <h1
      class="app-title"
    >{{(config.i18n[sel_lang] && config.i18n[sel_lang].welcomeTitle) || config.i18n[config.app.fallback_lang].welcomeTitle}}</h1>
    ...
    <div v-if="app.supportedLanguageCodes && app.supportedLanguageCodes.length > 0">
      ...
      >{{ app.defaultLanguageCode | toLang }}</span>

      <span
        @click="sel_lang = language"
        class="language-picker"
        v-for="(language, i) in app.supportedLanguageCodes"
        :key="i"
        :class="{'picked': sel_lang == language}"
      >{{ language | toLang }}</span>
    </div>
  </div>
</template>
